/* Fasig-Tipton market index — how a sire's yearlings have SOLD.
 *
 ...
 * biases and call them insight. At a yearling sale that circularity is worse
 * than at a 2YO sale, not better: with no clock and no race record, price is
 * *all* opinion.
 *
 * It stays because it's genuinely useful in the pavilion: knowing a sire's
 * yearlings typically bring $200K medians at Saratoga tells you where the
 * bidding will go. That's a budgeting fact, not a quality judgement.
 *
 * The racing basis for Pedigree comes from BloodHorse — see js/bloodhorse.js.
 */
window.FT = window.FT || {};
...
FT.sires = (function () {
  ...
  function lookup(ctx, name) {
    ...
    if (!key) return null;
    var override = FT.store.getSireOverride(key);
    var bh = FT.bloodhorse.lookup(ctx.bhIndex, name);
