...
(function () {
  ...
  function refreshHorse(key) {
    ...
    var vet = FT.store.vetStatus(key);
    document.querySelectorAll('[data-vet="' + CSS.escape(key) + '"]').forEach(function (sel) {
      if (document.activeElement === sel) return;
      sel.value = vet;
      FT.store.VET_STATES.forEach(function (s) { sel.classList.remove('vet-' + s); });
  ...
  /* Which control had focus, and where the caret was inside it. The grade
     ...
     digit. */
  var FOCUS_ATTRS = ['conf', 'ped', 'note', 'vet', 'sireRating'];
  ...
  function applyRemoteChanges() {
    var pending = FT.sync.pendingIds();
    ...
    var res = FT.store.applyRemote(FT.sync.data(), pending);
  ...
  function renderShare() {
    if (!FT.sync.configured) return;
    var st = FT.sync.status();
  ...
  function contributeIfFirstJoin(res) {
    ...
    FT.store.localOps().forEach(FT.sync.push);
    FT.sync.flush();
  }
  ...
  function wireGate() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      FT.sync.unlock(input.value).then(function (res) {
        input.value = '';
  ...
  function wireShare() {
    ...
    $('shareName').value = FT.sync.identity();
    wireGate();
    // 'locked' means configured but no code has ever been accepted here.
    showGate(FT.sync.status() === 'locked');

    FT.sync.onChange(function (what) {
      ...
      if (FT.sync.status() === 'locked') showGate(true);
    });
    ...
    $('shareName').addEventListener('change', function (e) {
      FT.sync.setIdentity(e.target.value);
    });
  ...
  FT.app = {
    ...
    /* Exposed so a shared-data change can be replayed by hand: stub
       FT.sync.data() with a snapshot and call this to watch it land. */
    applyRemoteChanges: applyRemoteChanges,
    contributeIfFirstJoin: contributeIfFirstJoin,
    /* Exposed so a deploy can be simulated: FT.app.checkForUpdate() after
