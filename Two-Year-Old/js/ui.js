...
OBS.ui = (function () {
  ...
  function saleHistoryHtml(h, state) {
    ...
    if (state.keeneland === 'unavailable') {
      note = '<div class="hist-line dim">OBS sales only — run <code>node serve.js</code> ' +
             'to include Keeneland.</div>';
  ...
  function detailHtml(h, ctx, activeMedia) {
    ...
    return '' +
    '<tr class="detail" data-detail-for="' + esc(h.key) + '"><td colspan="14"><div class="detail-inner">' +
